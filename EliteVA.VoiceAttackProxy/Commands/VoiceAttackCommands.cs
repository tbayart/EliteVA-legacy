...
namespace EliteVA.VoiceAttackProxy.Commands
{
    public class VoiceAttackCommands
    {
        private readonly dynamic _proxy;
        private readonly ILogger<VoiceAttackCommands> _log;

        internal VoiceAttackCommands(dynamic proxy, IServiceProvider services)
        {
            _proxy = proxy;
            _log = services.GetService<ILogger<VoiceAttackCommands>>();
        }
        ...
        {
            _log.LogTrace("Invoking command '{Name}'", commandName);
        ...
        {
            _log.LogTrace("Invoking command '{Name}'", identifier.ToString());
